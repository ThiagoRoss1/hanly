"""Local-only Windows packaging feasibility evidence for HAN-3.
...
def _packaged_probe() -> int:
    ...
    print("HAN-3 PACKAGED STARTUP PROBE")
    print(
...
def _packaged_ocr_probe() -> int:
    ...
    print("HAN-3 PACKAGED PADDLEOCR STARTUP PROBE")
    print(
        f"platform={platform.system()} python={platform.python_version()} "
        f"executable={sys.executable}"
    )
...
def main() -> int:
    ...
    if len(sys.argv) > 1:
        print("usage=han3_packaging_feasibility.py [--packaged-probe|--packaged-ocr-probe]")
        return 2

    platform_name = platform.system()
    print("HAN-3 PACKAGING FEASIBILITY SPIKE")
    print(
